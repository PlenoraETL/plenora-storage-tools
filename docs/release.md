...
La prima release riguarda Rust e CLI con S3-compatible, SFTP e FTP. Gli
...
3. `bash scripts/prepare-fixtures.sh`, quindi
   `docker compose run --rm --no-deps storage-rust`: tutti i test, senza skip,
...
5. `cargo audit --deny warnings` sul lockfile definitivo; usare cargo-audit
   ...
   Eseguire anche `cargo deny --locked check` con cargo-deny 0.20.2.
...
8. Riunire i due target sotto `dist/0.1.0/` e chiudere con
   ...
   `qualification.json`, `cli-regressions.json` e, per Linux, `commit-faults.json`.
...
permettono di isolare una qualifica sulla VM. Python 3.10 o successivo è richiesto
...
`no hash listed`. La verifica successiva estrae i cinque archivi in una
directory temporanea, compila ed esegue un consumer dei quattro crate Rust,
