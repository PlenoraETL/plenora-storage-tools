...
mod tests {
    ...
    async fn negotiate_stores_params_correctly() {
        ...
        // Regression: encryption-required Samba rejects sessions when this
        // implemented capability is missing, even for SMB 3.1.1.
        let sent = mock.sent_message(0).unwrap();
        let mut cursor = ReadCursor::new(&sent);
        Header::unpack(&mut cursor).unwrap();
        let request = NegotiateRequest::unpack(&mut cursor).unwrap();
        assert_ne!(request.capabilities.0 & Capabilities::ENCRYPTION, 0);
    }
